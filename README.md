...
2. 프로젝트 생성이 끝나면 좌측 메뉴 **SQL Editor** 로 이동 → `supabase/schema.sql` 파일 내용을 전체 복사해서 붙여넣고 **Run**
   - 소개(연혁), 코인 교환 목록, 환율 설정, 교환 신청 내역까지 테이블 4개가 한 번에 만들어집니다.
...
1. `js/supabase-config.js` 에서 `ADMIN_NOTIFY_EMAIL` 값을 실제 수신할 이메일 주소로 변경
...
- 회사 주소, 사업자등록번호 등 회사정보(현재 예시 문구가 들어가 있습니다)
